...
panel.waterfallchart <- 
  function(x, y, box.ratio = 1, box.width = box.ratio / (1 + box.ratio),
           horizontal = FALSE, origin = 0, reference = TRUE,
           groups = NULL, summaryname = NULL,
    ...
    if(is.null(groups))
      if(is.null(summaryname))
        ...
        groups = rep(summaryname, length(x))
    grplst <- sort(unique(groups))
    baseline <- rep(origin, (l = length(x) + length(grplst)) + 1)
    if (horizontal) {
      data <- merge(data.frame(x, y, groups,
                               groupsy = y)[order(groups, y), ],
                    data.frame(x = rep(NA, length(grplst)),
                               y = rep(NA, length(grplst)),
                               groups = grplst, groupsy = grplst),
                    all=TRUE)
      ...
      for(i in 2:l-1)
        panel.lines(y = c(i, i+1), x = baseline[i+1], col = border,
                    lty = lty, lwd = lwd)      
    } else {
      data <- merge(data.frame(x, y, groups,
                               groupsx = x)[order(groups, x), ],
                    data.frame(x = rep(NA, length(grplst)),
                               y = rep(NA, length(grplst)),
                               groups = grplst, groupsx = grplst),
                    all=TRUE)
      ...
      for(i in 2:l-1)
        panel.lines(x = c(i, i+1), y = baseline[i+1], col = border,
                    lty = lty, lwd = lwd)
    }
  }

waterfallchart <- function(x, data, ...) UseMethod("waterfallchart")
    ...
    if (any(!is.na(x) & !is.na(y))) {
      ...
      if (horizontal) {
        ...
        list(ylim = levels(factor(data$groupsy, levels=data$groupsy)),
             yat =  sort(unique(as.numeric(data$groupsy,
                                           levels=data$groupsy))),
             xlim ={ for(i in 1:(l-1)) {
                       if(!is.na(data[i,]$x))
                         baseline[i+1] <- baseline[i] + data[i,]$x
                       else
        ...
        list(xlim = levels(factor(data$groupsx, levels=data$groupsx)),
             xat =  sort(unique(as.numeric(data$groupsx,
                                           levels=data$groupsx))),
             ylim ={ for(i in 1:(l-1)) {
                       if(!is.na(data[i,]$y))
                         baseline[i+1] <- baseline[i] + data[i,]$y
                       else
                         baseline[i+1] <- baseline[i]
                     }
                     range(baseline) },
