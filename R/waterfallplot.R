...
waterfallplot.default <-
  function(height, width=1, space=NULL, names.arg=NULL, horiz=FALSE,
           density=NULL, angle=45, col=NULL, border=par("fg"), main=NA,
           sub=NA, xlab=NULL, ylab=NULL, xlim=NULL, ylim=NULL, xpd=TRUE,
  ...
  #  We're going to use this a lot, so we may as well save it
  l <- length(height)

  #  Figure out the summary, and reverse direction of everything if the
  #  summary is requested and reversed
  if(summary==TRUE) {
  ...
  if(horiz == TRUE) {
    ...
    expaxis <- 2
    for(i in 1:l)
      ticks.vec[i] <- (baseline[i] + topline[i]) / 2
  } else {
    for(i in 1:l)
      ticks.vec[i] <- (leftline[i] + rightline[i]) / 2  
  }
  ...
  #  Build the chart from the ground up
  if(plot == TRUE) {
    if(add == FALSE)
      plot.new()
    plot.window(xlim=xlim, ylim=ylim, ...)
    for(i in 1:l)
      rect(leftline[i], baseline[i], rightline[i], topline[i],
           density = density.vec[i], angle = angle.vec[i], col=
           col.vec[i], border=border.vec[i], xpd=xpd, ...)
    if(horiz == TRUE)
      for(i in 1:(l - 1))
        lines(c(leftline[i], leftline[i]),
              c(baseline[i], topline[i+1]), ...)
    else
      for(i in 1:(l - 1))
        lines(c(rightline[i], leftline[i+1]),
              c(topline[i], baseline[i+1]), ...)
    if(!is.null(names.arg) && axisnames) 
      axis(expaxis, at = ticks.vec, labels = names.arg, lty =
           axis.lty, cex.axis = cex.names, ...)
    if(axes == TRUE)
